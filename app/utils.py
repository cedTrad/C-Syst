...
def processing(portfolio_data: dict, positions_data: dict, open_orders_data: dict, get_price):
    ...
    return positions, r_risk, waterfall_fig
...
def ohlc(positions, selected_symbol, get_ohlc_data):
    ohlc_fig = go.Figure()
    if selected_symbol:
        start_date = datetime.now() - timedelta(days=7)
        end_date = datetime.now()
        interval = '1h'
        ohlc_data = get_ohlc_data(selected_symbol, start_date, end_date, interval)
        if ohlc_data:
